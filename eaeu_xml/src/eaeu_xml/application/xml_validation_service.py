...
class XmlValidationService:
    ...
    def validate(self, xml_text, *, engine, transaction_code, message_code, mode):
        ...
        if root.tag != f"{{{ns.SOAP}}}Envelope":
            issues.append(XmlDiagnostic("ERROR", "D5_SOAP_ENVELOPE_NAMESPACE", "Ожидается SOAP 1.2 Envelope.", "Envelope"))
            return XmlValidationResult(tuple(issues))
        header = root.find(f"{{{ns.SOAP}}}Header")
        body = root.find(f"{{{ns.SOAP}}}Body")
        if header is None or body is None:
            issues.append(XmlDiagnostic("ERROR", "D5_ENVELOPE_PART", "SOAP Envelope требует Header и Body.", "Envelope"))
            return XmlValidationResult(tuple(issues))
        ...
        if known != expected_order:
            issues.append(XmlDiagnostic("ERROR", "D5_HEADER_ORDER", "Нарушен нормативный порядок элементов SOAP Header.", "Header"))
        present = set(known)
        ...
        action = header.findtext(f"{{{ns.WSA}}}Action")
        if action:
            try:
                ...
                if parsed.serialize() != expected.serialize():
                    issues.append(XmlDiagnostic("ERROR", "D5_ACTION_UNEXPECTED", "Action не соответствует выбранному PROCESS/TRN/MSG.", "Header/Action"))
            except Exception as error:
                issues.append(XmlDiagnostic("ERROR", getattr(error, "code", "D5_ACTION_FORMAT"), getattr(error, "message", str(error)), "Header/Action"))
        ...
        if structure.root_element and children:
            ...
            if children[0].tag != expected_tag:
                issues.append(XmlDiagnostic("ERROR", "BODY_ROOT_UNEXPECTED", "Корневой элемент Body не соответствует выбранному сообщению.", "Body"))
